...
const globalState = read('src/global-state.js').replace(/^export /gm, '');
...
if (!process.argv.includes('--preview-only')) {
  ...
  const exportsBlock = /\nexport \{\s*TYPERT_REMOTE,\s*WHALE_FRAME_SCHEMA,\s*WHALE_WATCH_DESCRIPTOR\s*\};?\s*$/;
  if (!exportsBlock.test(remote)) throw new Error('Unexpected bridge contract exports; refusing a broken bundle');
